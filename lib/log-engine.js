...
class LogFile {
  ...
  wants(line, levelFilter) {
    return matchesLevelFilter(line, levelFilter);
  }
   ...
   */
  async jumpScan({ fromLine, count, levelFilter, collect, maxScanLines }) {
      ...
      while (collect.length < count && scanned < maxScanLines) {
        ...
        if (bytesRead === 0) {
          if (pending && pending.length) {
            ...
            scanned += 1;
            if (lineNo >= fromLine && this.wants(line, levelFilter) && collect.length < count) {
        ...
        if (lastNl === -1) {
          ...
          if (buf.length > MAX_PENDING_BYTES) {
            scanned += 1;
            const line = clipLine(makeDecoder(this.encoding).decode(buf));
            if (lineNo >= fromLine && this.wants(line, levelFilter) && collect.length < count) collect.push({ no: lineNo, text: line });
            lineNo += 1;
          } else {
...
class LogEngine {
  ...
  async page(p) {
    ...
    const collect = [];
    const result = await f.jumpScan({ fromLine, count, levelFilter, collect, maxScanLines: MAX_PAGE_SCAN_LINES });
    return {
      fileId: f.id,
      epoch: f.epoch,
      lines: collect,
