 ...
 *
 * All file IO here uses node:fs directly (byte-range reads at chunk
 * granularity). This is the documented fallback (README.md, and issue
 * vastsa/PI-Desktop#90): once the host exposes fs.readRange/fs.stat the
 * adapter can switch without touching this module's contract.
 *
 * Design:
...
const HANDLE_RETIRE_TIMEOUT_MS = 5000;
...
/**
 * Reject patterns that commonly cause catastrophic backtracking. This is a
 * conservative heuristic, not a full ReDoS analyzer — long lines already
 * stress `RegExp`, and the plugin process is single-threaded.
 */
function assertSafeRegex(source) {
  if (source.length > MAX_REGEX_LEN) {
    throw new ApiError("INVALID_ARGUMENT", `regex too long (max ${MAX_REGEX_LEN} chars)`);
  }
  ...
  if (nestedQuantifier) {
    throw new ApiError(
      "INVALID_ARGUMENT",
      "regex rejected: nested quantifiers may hang (e.g. (a+)+)",
    );
  }
}
...
class LogFile {
  ...
  async retireHandle(handle) {
    if (!handle) return;
    const deadline = Date.now() + HANDLE_RETIRE_TIMEOUT_MS;
  ...
  indexTrailingLine(buf, bufStart) {
    ...
    if (m) this.stats[LEVEL_MAP[m[1]]] += 1;
   ...
   */
  async jumpScan({ fromLine, count, levelSet, collect, maxScanLines }) {
      ...
      while (collect.length < count && scanned < maxScanLines) {
        ...
        if (bytesRead === 0) {
          if (pending && pending.length) {
            ...
            scanned += 1;
            if (lineNo >= fromLine && this.wants(line, levelSet) && collect.length < count) {
        ...
        if (lastNl === -1) {
          ...
          if (buf.length > MAX_PENDING_BYTES) {
            scanned += 1;
            const line = clipLine(makeDecoder(this.encoding).decode(buf));
            if (lineNo >= fromLine && this.wants(line, levelSet) && collect.length < count) collect.push({ no: lineNo, text: line });
            lineNo += 1;
          } else {
...
class LogEngine {
  constructor({ dataPath, capabilities }) {
  ...
  async setRoot(p) {
    ...
    if (!raw) throw new ApiError("INVALID_ARGUMENT", "path required");
    let real;
    try {
      real = await fsp.realpath(raw);
    } catch {
      real = path.resolve(raw);
    }
    const st = await fsp.stat(real).catch(() => null);
    if (!st || !st.isDirectory()) {
      throw new ApiError("INVALID_ARGUMENT", "selected path is not a directory");
    }
    this.userRoot = real;
    return { root: this.userRoot };
  }

  /**
   * Resolve `target` and refuse anything outside the current grant.
   * `allowDir` is true for listDir (the root itself); openFile needs a file.
   */
  async resolveUnderRoot(target, { allowDir = false } = {}) {
    if (!this.userRoot) {
      throw new ApiError("PERMISSION_DENIED", "no directory selected; pick a folder first");
    }
    const resolved = path.resolve(String(target || ""));
    if (!resolved) throw new ApiError("INVALID_ARGUMENT", "path required");
    let abs = resolved;
    try {
      abs = await fsp.realpath(resolved);
    } catch {
      // Missing path: lexical check still rejects obvious escapes.
    }
    const root = this.userRoot;
    const rel = path.relative(root, abs);
    if (rel === "") {
      if (!allowDir) {
        throw new ApiError("PERMISSION_DENIED", "expected a file under the selected directory");
      }
      return abs;
    }
    if (rel.startsWith("..") || path.isAbsolute(rel)) {
      throw new ApiError("PERMISSION_DENIED", "path outside the selected directory");
    }
    return abs;
  }
  ...
  async listDir(dirPath) {
    const abs = await this.resolveUnderRoot(dirPath, { allowDir: true });
    let entries;
    try {
      entries = await fsp.readdir(abs, { withFileTypes: true });
    } catch (err) {
      throw new ApiError("NOT_FOUND", `cannot read directory: ${err.message}`);
    }
    const out = [];
    for (const ent of entries) {
      if (out.length >= 2000) break;
      // File picker only: never surface subdirectories, so the UI cannot walk
      // deeper (or sideways via symlinks the host would have denied).
      if (!ent.isFile()) continue;
      const full = path.join(abs, ent.name);
      try {
        const st = await fsp.stat(full);
        out.push({ name: ent.name, path: full, isDirectory: false, size: st.size, mtimeMs: st.mtimeMs });
      } catch {
  ...
  /** Open without the grant check — used by restoreState for previously picked files. */
  async openFileInternal(filePath) {
    if (!filePath) throw new ApiError("INVALID_ARGUMENT", "path required");
    for (const f of this.files.values()) {
      if (f.path === filePath) return this.snapshot(f);
    }
  ...
  async openFile(filePath) {
    const abs = await this.resolveUnderRoot(filePath, { allowDir: false });
    return this.openFileInternal(abs);
  }
  ...
  async page(p) {
    ...
    let levelSet = null;
    if (Array.isArray(p.levels) && p.levels.length && p.levels.length < 5) {
      levelSet = new Set(p.levels.map(String));
    }
    const collect = [];
    const result = await f.jumpScan({ fromLine, count, levelSet, collect, maxScanLines: MAX_PAGE_SCAN_LINES });
    return {
      fileId: f.id,
      epoch: f.epoch,
      lines: collect,
  ...
  async poll(p) {
    ...
    if (replaced) {
      const handle = await fsp.open(f.path, "r").catch(() => null);
      if (handle) {
  ...
  buildMatcher(p) {
    ...
    if (!query) throw new ApiError("INVALID_ARGUMENT", "query required");
    ...
    if (p.isRegex) {
      ...
      try {
        ...
        throw new ApiError("INVALID_ARGUMENT", `invalid regex: ${err.message}`);
      }
      return { query, regex: true, caseSensitive, test: (line) => re.test(line), source: query };
    }
    ...
    return {
      ...
      regex: false,
      caseSensitive,
      test: (line) => (caseSensitive ? line.includes(needle) : line.toLowerCase().includes(needle)),
      source: query,
    };
  }
  ...
  async restoreState() {
    ...
    for (const t of Array.isArray(data.tabs) ? data.tabs : []) {
      ...
      try {
        ...
        const opened = await this.openFileInternal(path.resolve(t.path));
...
module.exports = { LogEngine, ApiError, ENCODINGS };
